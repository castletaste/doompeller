...
void main() {
    ...
    test('many small sectors in a grid', () {
      ...
      for (var gx = 0; gx < 4; gx++) {
        for (var gy = 0; gy < 4; gy++) {
          ...
          final int y = gy * 128;
          b.solidLoop(
            <int>[x, y, x + 128, y, x + 128, y + 128, x, y + 128],
            s,
          );
        }
      }
  ...
  group('degenerate and hostile input', () {
    ...
    test('open (unclosed) sector boundary is reported, not silently dropped',
        () {
      ...
      b.line(v1: v0, v2: v1, right: b.sidedef(sector: s, middle: 'STARTAN3'));
      b.line(v1: v1, v2: v2, right: b.sidedef(sector: s, middle: 'STARTAN3'));
      b.line(v1: v2, v2: v3, right: b.sidedef(sector: s, middle: 'STARTAN3'));
      final MapData map = b.build();
...
void _expectAgreementFor(MapData map, int sector, int? expectedArea) {
  ...
  final BspRegionSet regions =
      BspRegionBuilder(map, options).build(CheckBudget(4000000));
  final List<SectorLoopResult> loops =
      SectorLoopBuilder(map, options).buildAll(CheckBudget(4000000));
  ...
  expect(loops[sector].isComplete, isTrue,
  ...
  if (expectedArea != null) {
    expect(loopArea, closeTo(expectedArea.toDouble(), 1e-6),
  ...
  expect(bspArea, closeTo(loopArea, tolerance),
      reason: 'BSP and oracle disagree: gap of ${(bspArea - loopArea).abs()} '
